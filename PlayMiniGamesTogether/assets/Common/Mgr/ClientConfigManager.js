        ...
        return __extends(t, e),
            ...
            t.prototype.LoadStartInfo = function () {
                ...
                if (isgoServer) {
                    ...
                    if (this.ClientConfig.channel_id == "tgbot") {
                        cc.sys.platform = cc.sys.MOBILE_BROWSER
                    }
                    var click_id = app.ClientConfigManager().getLocalUrlDataByName("click_id");
                    if (click_id) {
                        this.ClientConfig.click_id = click_id
                    }
                    else {
                        this.ClientConfig.click_id = 0
                    }
                    var ttclid = app.ClientConfigManager().getLocalUrlDataByName("ttclid");
                    if (ttclid) {
                        this.ClientConfig.ttclid = ttclid
                    }
                    else {
                        this.ClientConfig.ttclid = 0
                    }
                    var language = app.ClientConfigManager().getLocalUrlDataByName("language");
                    this.ClientConfig.language = language
                    if (language == LanguageDefine.default.enus) {
                        // app.LanguageManager().SetLanguage(LanguageDefine.default.enus)
                        app.LocalDataManager().SetConfigProperty("SysSetting", "Language2", language)
                    }
                    else if (language == LanguageDefine.default.zhcn) {
                        //app.LanguageManager().SetLanguage(LanguageDefine.default.zhcn)
                        app.LocalDataManager().SetConfigProperty("SysSetting", "Language2", language)
                    }
                    else {
                        //app.LanguageManager().SetLanguage(LanguageDefine.default.enus)
