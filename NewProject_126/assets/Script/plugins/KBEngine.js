(function (scope) {
	...
	KBEngine.INFO_MSG = function (s) {
		if (KBEngine.is_publish === KBEngine.PUBLISH) return;
		console.info(s);
	};
	...
	KBEngine.Event = function () {
		...
		this.register = function (evtName, classinst, callback) {
			var callbackfn = callback;
			if (callbackfn === undefined) {
				KBEngine.ERROR_MSG('KBEngine.Event::fire: not found callback(' + classinst + ")!" + callback);
				return;
			}
		...
		this.fire = function () {
			...
			for (i = 0, len = evtlst.length; i < len; i++) {
				var info = evtlst[i];
				if (arguments.length < 1) {
	...
	KBEngine.MemoryStream = function (size_or_buffer) {
		// KBEngine.MemoryStream.prototype.init.call(this, size_or_buffer);
		this.rpos = 0;
		this.wpos = 0;
		this.init(size_or_buffer);
	};

	KBEngine.MemoryStream.prototype = {
		constructor: KBEngine.MemoryStream,

		init: function (size_or_buffer) {
			if (size_or_buffer instanceof ArrayBuffer) {
				this.buffer = size_or_buffer;
			} else {
				this.buffer = new ArrayBuffer(size_or_buffer); // 创建一个二进制数据缓冲区，缓冲区大小为size_or_buffer个字节
			}
		...
		readInt16: function () {
			...
			if (v >= 32768) { // ‭  1000 0000 0000 0000‬
		...
		readUint16: function () {
			...
			this.rpos += 2;
			return ((buf[1] & 0xff) << 8) + (buf[0] & 0xff); // 低字节在前，高字节在后
		},
		...
		readStringUTF8: function (skip) {
			var buf = new Uint8Array(this.buffer, this.rpos, skip);
			if (!!skip) {
				this.rpos += skip;
			}
			return KBEngine.utf8ArrayToString(buf);
		},
		...
		readStringGBK: function (skip) {
			...
			while (true) {
				if (buf[i] === 0) {
					i++;
					var arr = buf.slice(0, i);
					// s = new TextDecoder('gbk').decode(arr);
					break;
				}
				i++;
				if (this.rpos + i >= this.buffer.byteLength) {
					throw (new Error("KBEngine.MemoryStream::readStringGBK: rpos(" + (this.rpos + i) + ") >= " + this.buffer.byteLength + " overflow!"));
				}
			}
			this.rpos += skip;
			return s;
		...
		writeString: function (v) {
			...
			var i = 0;
			var utf8 = KBEngine.stringToUTF8Bytes(v);
			for (var idx = 0, len = utf8.length; idx < len; idx++) {
				buf[i++] = utf8[idx];
			}
	...
	KBEngine.Bundle.prototype = {
		...
		send: function (network) {
			network.send(this.stream.getbuffer());
		},
		writeInt8: function (v) {
		...
		writeString: function (v, n) {
			var utf8 = KBEngine.stringToUTF8Bytes(v);
			var num = utf8.length;
			var len = n || num + 1; // +1表示加上字符串结尾的\0
	...
	KBEngine.KBEngineApp = function () {
		...
		this.ip = null;
		this.port = null;
		this.onopencb = null;	// 连接服务器成功后的回调
	...
	KBEngine.KBEngineApp.prototype = {
		...
		connect: function (cb) {
			// console.assert(this.socket === null, "Assertion of socket not is null"); //打包微信小游戏使用console.assert会报错
			if (cb !== undefined) {
				this.onopencb = cb;
			}
			...
			if (this.wssAddr) {
				...
				if (!!!this.ip || !!!this.port || this.ip.length === 0) {
					KBEngine.ERROR_MSG('wrong ip or port: ' + this.ip + " " + this.port);
					return;
				}
				addr = "ws://" + this.ip + ":" + this.port;
			}
			KBEngine.INFO_MSG(addr);
			this.addr = addr;
			try {
				this.socket = new WebSocket(addr);
			} catch (e) {
				KBEngine.ERROR_MSG('WebSocket init error!');
				KBEngine.ERROR_MSG(e);
				return;
			}
			var socket = this.socket;
			socket.binaryType = "arraybuffer"; 							// 指定收到的数据是arrayBuffer类型
		...
		setOncloseCallback: function (cb) {
			this.onclosecb = cb;
		},
		...
		onmessage: function (msg) {
			if (this.stream !== null) {
				this.stream.init(msg.data);
			} else {
				this.stream = new KBEngine.MemoryStream(msg.data);
			}
			...
			// 读取数据内容
			this.net.handleMessage(mainCmdID, subCmdID, stream);
		},
		...
		onerror_before_onopen: function (evt) {
			KBEngine.ERROR_MSG('beforeconnect error:' + evt.data);
			this.close();
		},

		onerror_after_onopen: function (evt) {
			KBEngine.ERROR_MSG('afterconnect error:' + evt.data);
		},
	...
	KBEngine.destroyApp = function (app) {
		if (!app) return;
		app.close();
	};
	/*-----------------------------------------------------------------------------------------
	...
	KBEngine.CmdInfo = function (target, funcName, protocol) {
		this.target = target;
		this.funcName = funcName;
		this.protocol = protocol;
	};

	KBEngine.Net = function (network) {
	...
	scope.CMD_Info = {
		...
		write2bundle: function (bundle, wDataSize, cbCheckCode, cbMasterOrder) {
			bundle.writeUint16(wDataSize);
			bundle.writeUint8(cbCheckCode);
			bundle.writeUint8(cbMasterOrder);
		}
	};
	...
	scope.CMD_Command = {
		...
		write2bundle: function (bundle, wMainCmdID, wSubCmdID) {
			bundle.writeUint16(wMainCmdID);
			bundle.writeUint16(wSubCmdID);
		}
	};
	...
	KBEngine.Net.prototype = {
		...
		registerServer: function () {
			...
			var callbackName = arguments[1];
			var protocol = arguments[2];

			var mainCmdID = protocol.mainCmdID;
			var subCmdID = protocol.subCmdID;

			var funcDic = this._callClientFuncs[mainCmdID];
			if (typeof funcDic === "undefined") {
				funcDic = {};
				this._callClientFuncs[mainCmdID] = funcDic;
			}
			funcDic[subCmdID] = new KBEngine.CmdInfo(target, callbackName, protocol);
		},

		handleMessage: function (mainCmdID, subCmdID, stream) {
			// KBEngine.INFO_MSG("r " + mainCmdID + " " + subCmdID);
			if (!this._callClientFuncs[mainCmdID] || !this._callClientFuncs[mainCmdID][subCmdID]) {
				KBEngine.ERROR_MSG("mainCmdID: " + mainCmdID);
				KBEngine.ERROR_MSG("subCmdID: " + subCmdID);
				KBEngine.INFO_MSG("__________________________________");
				return;
			}
			var funcInfo = this._callClientFuncs[mainCmdID][subCmdID];
			var target = funcInfo.target;
			var funcName = funcInfo.funcName;
			var protocol = funcInfo.protocol;
			var args = [];
			protocol.createFromStream(stream, args);

			if (args.length <= 4) {
				target[funcName].apply(target, args);
			} else {
				var data = {};
				for (var i = 0, len = protocol.argTypes.length; i < len; i++) {
					data[protocol.argTypes[i].name] = args[i];
				}
				target[funcName].call(target, data);
			}
		},
		...
		registerClient: function () {
			...
			var funcInfo = new KBEngine.CmdInfo(target, funcName, protocol);
			this._funcName2Info[funcName] = funcInfo;
		},
		/**
		 * 发送数据到服务端
		 * 第一个参数是要调用的接口名称
		...
		callServer: function () {
			...
			var funcInfo = this._funcName2Info[funcName];
			var protocol = funcInfo.protocol;
			var funcArgs = Array.prototype.splice.call(arguments, 1); // 获得发送的参数
			var bundle = new KBEngine.Bundle();
			bundle.newMessage();
			var size = CMD_Info.size() + CMD_Command.size() + protocol.size(funcArgs); 	// 计算待发送数据的大小。单位：字节
			CMD_Info.write2bundle(bundle, size, 0, 65);
			CMD_Command.write2bundle(bundle, protocol.mainCmdID, protocol.subCmdID);
			protocol.write2bundle(bundle, funcArgs);
			bundle.send(this.network);
		},

		removeServer: function (main_cmd_id, sub_cmd_id) {
			var func_dic = this._callClientFuncs[main_cmd_id];
			if (!func_dic) {
				return;
			}
			if (func_dic[sub_cmd_id]) {
				delete func_dic[sub_cmd_id];
			}
		},

		removeClient: function (main_cmd_id, sub_cmd_id) {
			var func_dic = this._callServerFuncs[main_cmd_id];
			if (!func_dic) {
				return;
			}
			if (func_dic[sub_cmd_id]) {
	...
	scope.MD5 = {
		...
		safeAdd: function (x, y) {
			var lsw = (x & 0xFFFF) + (y & 0xFFFF);			// 16位及以下的位相加
	...
	KBEngine.EncryptData = function (data) {
		return scope.MD5.md5(data);
	};

	KBEngine.EncryptBuffer = function (buffer) {
