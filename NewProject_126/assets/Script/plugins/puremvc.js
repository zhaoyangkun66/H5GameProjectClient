 ...
 * @constructor
 */
function Observer (notifyMethod, notifyContext, type)
{
...
Observer.prototype.notifyObserver = function (notification)
{
    ...
    if(this.type === 'mediator') {
        this.notify.apply(this.context, notification.body);
    } else {
 ...
 * @return {void}
 */
Notifier.prototype.sendNotification = function()
{
    var facade = this.getFacade();
    if(facade) {
        var notificationName = arguments[0];
        var args = [];
        for(var i = 0, len = arguments.length; i < len; i++) {
            args.push(arguments[i]);
        }
        facade.sendNotification.apply(facade, args);
    }
};
...
MacroCommand.prototype.execute = function(note)
{
    ...
    //     cmd.execute(note);
    // }
    for(var i = 0, len = this.subCommands.length; i < len; i++) {
        var ref = this.subCommands[i];
        var cmd = new ref;
        cmd.initializeNotifier(this.multitonKey);
        if(note.body && note.body.length > 0) {
            cmd.execute.apply(cmd, note.body);
        } else {
            cmd.execute(note);
        }
    }
...
/**
 * 添加代理。这一步只创建对象，不执行生命周期方法 。
 */
Facade.prototype.addProxy = function(proxy) {
 ...
 * @return {void}
 */
Facade.prototype.registerProxy = function()
{
    this.model.registerProxy();
};
 ...
 * @return {void}
 */
Facade.prototype.sendNotification = function()
{
    var notificationName = arguments[0];
    var args = [];
    ...
    this.notifyObservers(new Notification(notificationName, args));
};
...
View.prototype.registerMediator = function(mediator)
{
    ...
    // if(Object.hasOwnProperty.call(mediator, 'listNotificationInterests')) {
    ...
    if(interests && interests.length > 0) {
        ...
        for(var i = 0, len = interests.length; i < len; i++) {
            item = interests[i];
            observer = new Observer(item.callback, mediator, 'mediator');
            this.registerObserver(item.evtType, observer);
        }
    }
    // }
...
View.prototype.removeMediator = function(mediatorName)
{
    ...
    if(mediator)
    {
        ...
        for(var i = 0; i < interests.length; i++)
        {
            ...
            // interest
            this.removeObserver(interests[i].evtType, mediator);
        }
...
Model.prototype.addProxy = function(proxy) {
 ...
 * @param {puremvc.Proxy}
 */
Model.prototype.registerProxy = function()
{
    for(var proxyName in this.proxyMap) {
        this.proxyMap[proxyName].onRegister();
    }
};
...
Controller.prototype.executeCommand= function(note)
{
    ...
    if(note.body && note.body.length > 0) {
        commandInstance.execute.apply(commandInstance, note.body);
    } else {
...
function define (classInfo, traits, staticTraits)
{
    ...
    var doExtend    = 'function' === typeof classParent;
    var classConstructor;
    var classScope  = classInfo.scope || null;
    var prototype;
    ...
    if (className)
    {
        ...
        // 这里的功能是根据名称来划分名称空间，如org.game.ui就会创建出window.org.name.ui这样的级联关系
