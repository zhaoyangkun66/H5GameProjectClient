...
var o = require("../../../Common/Base/BaseForm")
    ...
    , UINameDefine = require("UINameDefine")
    , l = cc._decorator
    , p = l.ccclass
    , d = l.property
    , h = function (e) {
        ...
        return __extends(t, e),
            ...
            t.prototype.OnCreateInit = function () {
                ...
                this.RegEvent(i.GameEventDefine.GET_GongGao_LIST, this.RunTxtActIInit)
                this.RegEvent(i.GameEventDefine.GetGongGaoDetail, this.onGetGongGaoDetail)
            }
            ,
            t.prototype.OnShow = function () {
                ...
                this.gongGaoData = null
                this.tempTxtContent = null
                app.HNoticeManager().RequestGetGongGaoList()
            }
            ,
            ...
            t.prototype.OnClick = function (e, t) {
                if ("btn_GongGao" == e) {
                    if (this.gongGaoData) {
                        app.FormManager().ShowForm(UINameDefine.UINameDefine.UIGongGaoAll, this.gongGaoData);
                    }
                }
                else if ("view" == e) {
                    if (this.tempTxtContent) {
                        app.FormManager().ShowForm(UINameDefine.UINameDefine.UIGongGao, this.tempTxtContent);
                    }
                }
            ...
            t.prototype.RenderGameTypeTitle = function () {
                ...
                this.typeIconArr = [],
                    ...
                    a.forEach(function (n) {
                        ...
                        if (o) {
                            if (!s || l > 1) {
                                var i = app.LocalDataManager().GetConfigProperty("SysSetting" + GameTypeGlobal, "Language")
            ...
            t.prototype.OnSetLanguage = function () {
                for (var e = 0; e < this.typeIconArr.length; e++) {
                    var t = this.typeIconArr[e]
                        ...
                        , o = app.LocalDataManager().GetConfigProperty("SysSetting" + GameTypeGlobal, "Language")
            ...
            ,
            t.prototype.onGetGongGaoDetail = function (send) {
                for (let index = 0; index < this.gongGaoData.length; index++) {
                    const element = this.gongGaoData[index];
                    if (send.id == element.id) {
                        element.status = 1
                        break
                    }
                }
                this.updataGongGaoRedDot()
            },
            t.prototype.updataGongGaoRedDot = function () {
                let count = 0
                for (let index = 0; index < this.gongGaoData.length; index++) {
                    const element = this.gongGaoData[index];
                    if (element.status == 0) {
                ...
                if (count > 0) {
                    this.GetWndNode("node_home/gongGaoNode/btn_GongGao/GongGaoReaded").active = false
                    this.GetWndNode("node_home/gongGaoNode/btn_GongGao/GongGaoUnread").active = true
                    this.GetWndNode("node_home/gongGaoNode/btn_GongGao/GongGaoUnread/RedDot/label_num").getComponent(cc.Label).string = count
                }
                else {
                    this.GetWndNode("node_home/gongGaoNode/btn_GongGao/GongGaoReaded").active = true
                    this.GetWndNode("node_home/gongGaoNode/btn_GongGao/GongGaoUnread").active = false
                }
            },
            t.prototype.RunTxtActIInit = function (data) {
                if (!data) {
                    data = []
                }
                this.gongGaoData = data
                this.updataGongGaoRedDot()
                if (this.GetWndNode("node_home/gongGaoNode")) {
                    this.txtContent = this.GetWndNode("node_home/gongGaoNode/view/richtext").getComponent(cc.RichText)
                    this.txtContentview = this.GetWndNode("node_home/gongGaoNode/view")
                }
                if (!this.txtContentPool) {
                    this.txtContentPool = []
                }
                for (let index = 0; index < this.gongGaoData.length; index++) {
                    this.txtContentPool.push(this.gongGaoData[index])
                }
                this.txtContent.node.stopAllActions()
                this.RunTxtAct()
            },
            t.prototype.RunTxtAct = function () {
                ...
                if (this.txtContentPool.length > 0) {
                    this.txtContent.getComponent(RichTextCallBack.default).setRichtextStrCallBack({
                        str: this.txtContentPool[0].content,
                    ...
                    this.tempTxtContent = this.txtContentPool[0]
                    this.txtContentPool.splice(0, 1)
                    this.txtContent.node.x = this.txtContentview.width
                    let width = this.txtContentview.width + this.txtContent.node.width
                    let moveact = []
