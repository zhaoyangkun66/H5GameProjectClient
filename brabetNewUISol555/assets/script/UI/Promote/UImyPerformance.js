...
cc.Class({
    ...
    OnLoadInit() {
        this.RegEvent(i.GameEventDefine.newagent_myPerformance, this.onNewagent_myPerformance)
    ...
    OnEnable() {
        ...
        app.PromoteMainManager().ReqNewagent_myPerformance(this.getTime(Number(1)))
    },
    onNewagent_myPerformance(dataRoot) {
        ...
        for (var n = 0; n < data.length; n++) {
            ...
            if (o.direct_amount > 0) {
                let directPerformance = i.getChildByName("directPerformance").getComponent(cc.Button)
                let EventHandler = new cc.Component.EventHandler;
                EventHandler.target = this.node
                EventHandler.component = this.JS_Name
                EventHandler.handler = "PerformanceClickEvent"
                EventHandler.customEventData = JSON.stringify({ record_time: o.record_time, type: 1 })
                directPerformance.clickEvents = []
                directPerformance.clickEvents.push(EventHandler)
                i.getChildByName("directPerformance").getComponent(cc.RichText).string = "D. Direto <color=#DCB154><u>" + o.direct_amount + "</u></color>"
            }
            else {
                i.getChildByName("directPerformance").getComponent(cc.RichText).string = "D. Direto <color=#FFFFFF>" + o.direct_amount + "</color>"
            }
            if (o.other_amount > 0) {
                let otherPerformance = i.getChildByName("otherPerformance").getComponent(cc.Button)
                let EventHandler = new cc.Component.EventHandler;
                EventHandler.target = this.node
                EventHandler.component = this.JS_Name
                EventHandler.handler = "PerformanceClickEvent"
                EventHandler.customEventData = JSON.stringify({ record_time: o.record_time, type: 2 })
                otherPerformance.clickEvents = []
                otherPerformance.clickEvents.push(EventHandler)
                i.getChildByName("otherPerformance").getComponent(cc.RichText).string = "D. dos Outros <color=#DCB154><u>" + o.other_amount + "</u></color>"
            }
            else {
                i.getChildByName("otherPerformance").getComponent(cc.RichText).string = "D. dos Outros <color=#FFFFFF>" + o.other_amount + "</color>"
            }
            i.getChildByName("totalPerformance").getChildByName("totalPerformance").getComponent(cc.Label).string = o.total_amount
    ...
    scrollEvents(t) {
        ...
        if (isScrolledPast80Percent && this.n_IsSend) {
            ...
            app.PromoteMainManager().ReqNewagent_myPerformance({ start: dataRoot.start_date, end: dataRoot.end_date, page: this.n_page })
        }
    },
    PerformanceClickEvent(t, element) {
        var json = JSON.parse(element)
        app.FormManager().ShowForm(UINameDefine.UINameDefine.UImyPerformanceDetail, json);
    },
    OnClick(e, t, n) {
        ...
        if ("all" == e || "today" == e || "yesterday" == e || "week" == e || "week_last" == e || "month" == e || "month_last" == e) {
            app.PromoteMainManager().ReqNewagent_myPerformance(this.getTime(Number(n)))
        }
            ...
            if (nodeJianTou.scaleY == 1) {
                ...
                cc.tween(this.nodebottom).by(0.1, { y: -60 }).start()
            }
            else {
                nodeJianTou.scaleY = 1
                cc.tween(this.nodebottom).by(0.1, { y: 60 }).start()
            ...
            if ("btn_search" != e)
                ...
                if (i == this.InitDateStr || a == this.InitDateStr)
                    return;
                app.PromoteMainManager().ReqNewagent_myPerformance({
                    start: i,
                    end: a
                })
            }
