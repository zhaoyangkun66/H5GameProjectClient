    ...
    , u = function (e) {
        ...
        return __extends(t, e),
            ...
            t.prototype.OnClick = function (e, t, n) {
                if ("btn_HelpCoupon" == e) {
                    app.FormManager().ShowForm(c.UINameDefine.UIWithdrawalCouponDes)
                }
                else if ("nodeWithdrawalCoupon" == e) {
                    app.FormManager().ShowForm(c.UINameDefine.UIWithdrawalCoupon)
                    app.FormManager().CloseForm(c.UINameDefine.UIStoreAndCash)
                }
                else {
                    this.Log(e),
                        "btn_service" != e
            ...
            ,
            t.prototype.AmountNumberLabelEND = function () {
                if (GameTypeGlobal == 3) {
                    var e = Number(this.AmountNumberLabel.getComponent(cc.EditBox).string)
                    e = (e - e % 1000)
                    this.AmountNumberLabel.getComponent(cc.EditBox).string = "" + e
                    if (Number(e)) {
                        if (this.cash_rate) {
                            this.rateLabelNode.string = app.GameConfigManager().GetCurrency + " " + (Number(e) * this.cash_rate).toFixed(2);
                            var t = (Number(e) * (1 - this.cash_rate) / Number(this.UserWallerData.rate)).toFixed(2);
            ...
            t.prototype.txRequest = function () {
                // if (GameTypeGlobal == 2) {
                //     // if (!app.UserManager().GetUserWallet().isWithdrawalPassword) {
                //     //     app.FormManager().ShowForm(c.UINameDefine.UIWithdrawalPassword)
                //     //     return
                //     // }
                //     let self = this
                //     if (!this.WithdrawalPassword) {
                //         app.FormManager().ShowForm(c.UINameDefine.UIWithdrawalPassword2, function (params) {
                //             //app.SysNotifyManager().ShowToast(params);
                //             self.WithdrawalPassword = params
                //         })
                //         return
                //     }
                // }
                ...
                if ("" != this.AmountNumberLabel.getComponent(cc.EditBox).string) {
                    ...
                    else
                    {
                        this.isAgentTx ? app.CashOutManager().RequestAgentTx({
                            ...
                            amount: t,
                            is_usdt: o,
                            withdrawalPassword: this.WithdrawalPassword
                        })
                    }
            ...
            t.prototype.onGetUserWallet = function () {
                ...
                if (cc.find("view/content/nodeWithdrawalCoupon", this.node)) {
                    cc.find("view/content/nodeWithdrawalCoupon/couponBalance", this.node).getComponent(cc.Label).string = app.GameConfigManager().GetCurrency + " " + app.ScoreUtil().toFixed(app.UserManager().GetUserInfo.saldo)
                }
